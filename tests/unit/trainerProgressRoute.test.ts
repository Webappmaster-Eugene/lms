...
/**
 * POST /api/trainer-progress — единственная дверь, через которую пользователь
 * отмечает задачу тренажёра решённой. За записью в user-trainer-progress следует
 * хук начисления баллов, а баллы — это лидерборд и достижения.
 *
 * Ключевое здесь: сервер не верит клиенту. Браузер уже сравнил вывод с эталоном,
 * но запрос формирует тоже браузер, и отправить `output` с правильным ответом
 * может кто угодно из консоли. Поэтому маршрут заново читает задачу из БД и сам
 * прогоняет validateOutput. Если эта проверка исчезнет, лидерборд можно накрутить
 * одним fetch — и снаружи это будет выглядеть как обычное решение задачи.
 *
 * Payload подменён целиком: проверяется логика маршрута, а не работа ORM.
 * Обращения к БД (правильная ли коллекция, те ли условия выборки) видны
 * в аргументах моков, и они тоже проверяются — выборка без фильтра isPublished
 * открыла бы доступ к черновым задачам.
 */
...
describe('проверка тела запроса', () => {
  ...
  it('пустой output допустим — задача может не требовать вывода', async () => {
    // Здесь важно, что проверка отличает '' от отсутствия поля: `!output`
    // отвергал бы легитимный случай.
    find.mockImplementation(async ({ collection }: { collection: string }) =>
...
describe('ограничения размера', () => {
  ...
  it('слишком длинный вывод отвергается', async () => {
    // Без ограничения пользовательский цикл с console.log положил бы в БД
    // мегабайты на каждую попытку.
    const response = await POST(
...
describe('задача должна существовать и быть опубликованной', () => {
  ...
  it('выборка задачи фильтрует по isPublished', async () => {
    // Иначе решение черновой задачи засчитывается и приносит баллы до публикации.
...
describe('серверная проверка вывода — защита от накрутки', () => {
  ...
  it('эталон берётся из БД, а не из запроса', async () => {
    // Попытка передать свой expectedOutput не должна ни на что влиять.
    const response = await POST(
  ...
  it('к выводу применяется та же нормализация, что и на клиенте', async () => {
    // Иначе решение, зелёное в браузере, отвергается сервером — и пользователь
    // видит «всё совпало», но задача не засчитана.
    const response = await POST(post({ taskId: 'task-1', userCode: 'x', output: '\r\n 42  \n' }))
...
describe('запись прогресса', () => {
  ...
  it('существующая запись ищется по паре пользователь+задача', async () => {
    // Поиск только по задаче нашёл бы чужой прогресс и переписал его.
    await POST(post({ taskId: 'task-1', userCode: 'x', output: '42' }))
  ...
  it('запись всегда привязывается к пользователю из сессии', async () => {
    // Даже если в теле пришёл чужой user — он не должен попасть в данные.
