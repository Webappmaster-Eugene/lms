/**
 * seed-roadmaps.ts — создаёт визуальные роадмапы (узлы, связи, линковка курсов)
 * в стиле Miro-досок @eugene_galera (Frontend React / Backend Node.js).
 *
 * Идемпотентен: при повторном запуске удаляет старые узлы/ребра текущих
 * роадмапов и пересоздаёт их заново.
 *
 * Что делает:
 * 1. Находит или создаёт роадмапы (frontend-react, backend-nodejs).
 * 2. Транскрибирует контент Miro-досок: узлы (category/topic/subtopic)
 *    со стадиями (base → stage1 → stage2 → practice → advanced → growth),
 *    цветами (yellow/lime/white/gray/pink/red) и внутренними bullet-списками.
 * 3. Создаёт stub-курсы для каждого topic-узла, не имеющего реального курса,
 *    чтобы все узлы графа были кликабельны и показывали прогресс.
 * 4. Линкует существующие курсы (js-basics, react-fundamentals, node-basics)
 *    к соответствующим узлам.
 * 5. Создаёт ребра между узлами.
 *
 * Запуск: npx tsx src/seed-roadmaps.ts
 */
...
type NodeSeed = {
  ...
  /** Внутренний список под-тем — отображается внутри карточки на графе (Miro-стиль). */
  bullets: string[]
  /** Если задан — узел будет привязан к курсу с таким slug. */
  existingCourseSlug?: string
  /** Если true — создать stub-курс (1 секция + 1 демо-урок) и привязать к нему. */
  createStub?: boolean
}
...
const frontendNodes: NodeSeed[] = [
  // --- Старт ---
  {
    ...
    positionY: 0,
    icon: 'rocket',
    description: 'Старт обучения Frontend-разработке',
    order: 0,
    stage: 'start',
    color: 'pink',
    bullets: [],
  ...
  // --- База (1 неделя — повторяем базу) ---
  {
    ...
    positionY: 180,
    icon: 'file-code',
    description: 'Разметка, семантика, доступность',
    order: 1,
    stage: 'base',
    color: 'yellow',
    bullets: [
      'Теги, атрибуты, структура документа',
      'Семантическая разметка (header, main, article)',
      'Формы и валидация HTML5',
      'Таблицы и списки',
      'Доступность (a11y), ARIA-атрибуты',
      'Медиа (audio, video, picture)',
      'SEO-теги, OpenGraph, Schema.org',
    ],
    createStub: true,
  },
  {
    ...
    positionY: 180,
    icon: 'palette',
    description: 'Стилизация, адаптив, анимации',
    order: 2,
    stage: 'base',
    color: 'yellow',
    bullets: [
      'Селекторы, каскад, специфичность',
      'Box-model, позиционирование',
      'Flexbox',
      'Grid',
      'CSS-переменные',
  ...
  },
  {
    ...
    positionY: 180,
    icon: 'monitor',
    description: 'DOM, события, браузер',
    order: 4,
    stage: 'base',
    color: 'yellow',
    bullets: [
  ...
  // --- Стадия 1 (2–4 недели — разработка основы) ---
  {
    nodeId: 'fe-ts-build',
    label: 'TypeScript и сборка',
    nodeType: 'topic',
    positionX: 40,
    positionY: 460,
    icon: 'shield',
    description: 'Типизация, компиляция, бандлеры',
    order: 5,
    stage: 'stage1',
    color: 'lime',
    bullets: [
      'Базовые типы, интерфейсы',
      'Generics и Utility-типы',
      'tsconfig.json и strict-режим',
  ...
  {
    nodeId: 'fe-js-adv',
    label: 'JS — продвинутый',
    nodeType: 'topic',
    positionX: 300,
    positionY: 460,
    icon: 'cpu',
    description: 'Асинхронность, замыкания, прототипы',
    order: 6,
    stage: 'stage1',
    color: 'lime',
    bullets: [
      'Event Loop, микротаски',
      'Async/await, Promise chains',
      'Замыкания и область видимости',
      'Прототипы и наследование',
      'this, call / apply / bind',
      'ES6+ (Map, Set, Symbol, Proxy)',
      'Модули (ESM, CommonJS)',
    ],
    createStub: true,
  },
  {
    ...
    positionY: 460,
    icon: 'zap',
    description: 'Компоненты, состояние, хуки',
    order: 7,
    stage: 'stage1',
    color: 'lime',
    bullets: [
    ...
    existingCourseSlug: 'react-fundamentals',
  },
  {
    nodeId: 'fe-react-eco',
    label: 'Экосистема React',
    nodeType: 'topic',
    positionX: 820,
    positionY: 460,
    icon: 'puzzle',
    description: 'State-менеджеры, роутинг, формы',
    order: 8,
    stage: 'stage1',
    color: 'lime',
    bullets: [
  ...
  // --- Стадия 2 (Junior, 1 неделя — освоение инструментов) ---
  {
    nodeId: 'fe-layout',
    label: 'Вёрстка',
    nodeType: 'topic',
    ...
    positionY: 760,
    icon: 'layout',
    description: 'Pixel-perfect, адаптивность, кроссбраузерность',
    order: 9,
    stage: 'stage2',
    color: 'white',
    bullets: [
  ...
  {
    nodeId: 'fe-arch',
    label: 'Архитектура и код-ревью',
    nodeType: 'topic',
    positionX: 300,
    positionY: 760,
    icon: 'folder-tree',
    description: 'Паттерны, чистый код, ревью',
    order: 10,
    stage: 'stage2',
    color: 'white',
    bullets: [
  ...
  // --- Практика (Middle, 1–2 месяца) ---
  {
    nodeId: 'fe-real-project',
    label: 'Реальные проекты',
    nodeType: 'topic',
    positionX: 40,
    positionY: 1060,
    icon: 'rocket',
    description: 'Работа с клиентом, ТЗ, сроки',
    order: 13,
    stage: 'practice',
    color: 'pink',
    bullets: [
      'Сбор требований и анализ ТЗ',
  ...
  {
    nodeId: 'fe-complex-int',
    label: 'Сложные интеграции',
    nodeType: 'topic',
    positionX: 300,
    positionY: 1060,
    icon: 'network',
    description: 'WebSocket, GraphQL, OAuth',
    order: 14,
    stage: 'practice',
    color: 'pink',
    bullets: [
  ...
  },
  {
    ...
    positionY: 1060,
    icon: 'zap',
    description: 'Web Vitals, оптимизация',
    order: 15,
    stage: 'practice',
    color: 'pink',
    bullets: [
  ...
  // --- Advanced (Senior) ---
  {
    ...
    positionY: 1340,
    icon: 'workflow',
    description: 'Алгоритмы, system design, soft-skills',
    order: 17,
    stage: 'advanced',
    color: 'red',
    bullets: [
      'Алгоритмы и структуры данных',
      'System design для фронта',
      'Live-coding задачи',
  ...
  },
  {
    ...
    positionY: 1340,
    icon: 'book-open',
    description: 'CV, GitHub, LinkedIn',
    order: 18,
    stage: 'advanced',
    color: 'red',
    bullets: [
      'Структура резюме',
      'Описание достижений через метрики',
      'GitHub-профиль и pinned репозитории',
  ...
  {
    nodeId: 'fe-mentoring',
    label: 'Менторинг и рост',
    nodeType: 'topic',
    positionX: 780,
    positionY: 1340,
    icon: 'network',
    description: 'Наставничество, команда',
    order: 19,
    stage: 'advanced',
    color: 'red',
    bullets: [
  ...
  // --- Рост (дополнительные блоки) ---
  {
    nodeId: 'fe-growth-monorepo',
    label: 'Монорепо',
    nodeType: 'subtopic',
    positionX: 60,
    positionY: 1620,
    icon: 'box',
    description: 'Turborepo, Nx, pnpm workspaces',
    order: 20,
    stage: 'growth',
    color: 'gray',
    bullets: [],
  ...
  {
    nodeId: 'fe-growth-micro',
    label: 'Микрофронтенды',
    nodeType: 'subtopic',
    positionX: 280,
    positionY: 1620,
    icon: 'puzzle',
  ...
  {
    nodeId: 'fe-growth-wasm',
    label: 'WebAssembly',
    nodeType: 'subtopic',
    positionX: 500,
    positionY: 1620,
    icon: 'cpu',
    description: 'Rust/Go → WASM во фронте',
    order: 22,
    stage: 'growth',
    color: 'gray',
    bullets: [],
...
const backendNodes: NodeSeed[] = [
  // --- Старт ---
  {
    ...
    positionY: 0,
    icon: 'rocket',
    description: 'Старт обучения Backend-разработке',
    order: 0,
    stage: 'start',
    color: 'pink',
    bullets: [],
  ...
  // --- База ---
  {
    ...
    positionY: 180,
    icon: 'file-code',
    description: 'Основы JavaScript для сервера',
    order: 1,
    stage: 'base',
    color: 'yellow',
    bullets: [
  ...
  },
  {
    ...
    positionY: 180,
    icon: 'shield',
    description: 'TypeScript для Node',
    order: 2,
    stage: 'base',
    color: 'yellow',
    bullets: [
  ...
  },
  {
    ...
    positionY: 180,
    icon: 'server',
    description: 'Runtime, модули, npm',
    order: 4,
    stage: 'base',
    color: 'yellow',
    bullets: [
    ...
    existingCourseSlug: 'node-basics',
  },
  {
    nodeId: 'be-linux',
    label: 'Linux / Терминал',
    nodeType: 'topic',
    positionX: 840,
    positionY: 180,
    icon: 'terminal',
    description: 'Bash, процессы, права',
    order: 5,
    stage: 'base',
    color: 'yellow',
    bullets: [
      'Базовые команды (ls, cd, grep)',
      'Bash-скрипты',
      'Права доступа, chmod/chown',
      'Процессы, ps, top, htop',
      'SSH, scp, rsync',
      'Cron и systemd',
    ],
    createStub: true,
  ...
  {
    nodeId: 'be-algo',
    label: 'Алгоритмы и структуры',
    nodeType: 'topic',
    positionX: 40,
    positionY: 460,
    icon: 'cpu',
    description: 'Big O, деревья, графы',
    order: 6,
    stage: 'stage1',
    color: 'lime',
    bullets: [
  ...
  },
  {
    ...
    positionY: 460,
    icon: 'box',
    description: 'Модули, DI, декораторы',
    order: 7,
    stage: 'stage1',
    color: 'lime',
    bullets: [
  ...
  },
  {
    ...
    positionY: 460,
    icon: 'folder-tree',
    description: 'SOLID, Clean Architecture, DDD',
    order: 8,
    stage: 'stage1',
    color: 'lime',
    bullets: [
  ...
  {
    nodeId: 'be-sql',
    label: 'SQL и реляционные БД',
    nodeType: 'topic',
    positionX: 640,
    positionY: 460,
    icon: 'database',
    description: 'PostgreSQL, индексы, транзакции',
    order: 9,
    stage: 'stage1',
    color: 'lime',
    bullets: [
  ...
  },
  {
    ...
    positionY: 460,
    icon: 'database',
    description: 'MongoDB, Redis, ClickHouse',
    order: 10,
    stage: 'stage1',
    color: 'lime',
    bullets: [
      'MongoDB: документы, коллекции',
  ...
  // --- Стадия 2 (Junior) ---
  {
    ...
    positionY: 760,
    icon: 'box',
    description: 'Контейнеризация, compose',
    order: 11,
    stage: 'stage2',
    color: 'white',
    bullets: [
  ...
  {
    nodeId: 'be-orm',
    label: 'ORM и миграции',
    nodeType: 'topic',
    positionX: 440,
    positionY: 760,
    icon: 'database',
    description: 'Drizzle, Prisma, TypeORM',
    order: 13,
    stage: 'stage2',
    color: 'white',
    bullets: [
  ...
  },
  {
    ...
    positionY: 760,
    icon: 'network',
    description: 'RabbitMQ, Kafka, BullMQ',
    order: 15,
    stage: 'stage2',
    color: 'white',
    bullets: [
      ...
      'Kafka: топики, партиции',
      'Паттерны: pub/sub, work queue',
  ...
  // --- Практика (Middle) ---
  {
    ...
    bullets: [
      'Jest / Vitest',
      'Unit-тесты сервисов',
      'Integration: real БД (testcontainers)',
      'E2E через supertest',
  ...
  },
  {
    ...
    positionY: 1060,
    icon: 'monitor',
    description: 'Логи, метрики, трейсинг',
    order: 18,
    stage: 'practice',
    color: 'pink',
    bullets: [
  ...
  },
  {
    ...
    positionY: 1060,
    icon: 'shield',
    description: 'OWASP Top 10, защита API',
    order: 19,
    stage: 'practice',
    color: 'pink',
    bullets: [
  ...
  // --- Advanced (Senior) ---
  {
    nodeId: 'be-microservices',
    label: 'Микросервисы',
    nodeType: 'topic',
    positionX: 40,
    positionY: 1340,
    icon: 'network',
    description: 'Разделение, коммуникация, паттерны',
    order: 20,
    stage: 'advanced',
    color: 'red',
    bullets: [
  ...
  },
  {
    ...
    positionY: 1340,
    icon: 'zap',
    description: 'k6, Artillery, стресс-тесты',
    order: 22,
    stage: 'advanced',
    color: 'red',
    bullets: [
      'k6 или Artillery',
      'Load / Stress / Spike тесты',
  ...
  },
  {
    ...
    positionY: 1340,
    icon: 'workflow',
    description: 'System design, алгоритмы',
    order: 23,
    stage: 'advanced',
    color: 'red',
    bullets: [
  ...
  {
    nodeId: 'be-growth-ml',
    label: 'ML / AI в бэкенде',
    nodeType: 'subtopic',
    positionX: 40,
    positionY: 1620,
    icon: 'zap',
    description: 'LLM API, vector DB',
    order: 24,
    stage: 'growth',
    color: 'gray',
    bullets: [],
  ...
  {
    nodeId: 'be-growth-web3',
    label: 'Web3 / Blockchain',
    nodeType: 'subtopic',
    positionX: 260,
    positionY: 1620,
    icon: 'network',
    description: 'Ethereum, смарт-контракты',
    order: 25,
    stage: 'growth',
    color: 'gray',
    bullets: [],
  ...
  {
    nodeId: 'be-growth-rust',
    label: 'Rust для бэка',
    nodeType: 'subtopic',
    positionX: 480,
    positionY: 1620,
    icon: 'server',
    description: 'Производительность, safety',
    order: 26,
    stage: 'growth',
    color: 'gray',
    bullets: [],
  ...
  {
    nodeId: 'be-growth-arch',
    label: 'Cloud-native design',
    nodeType: 'subtopic',
    positionX: 700,
    positionY: 1620,
    icon: 'cloud',
    description: 'Serverless, edge',
    order: 27,
    stage: 'growth',
    color: 'gray',
    bullets: [],
...
async function seedRoadmaps() {
  ...
  // --- Чистим старые узлы и рёбра (идемпотентность) ---
  ...
  // --- Создаём узлы для обоих роадмапов ---
...
/**
 * Создаёт узлы одного роадмапа, попутно линкуя их к курсам:
 * - existingCourseSlug → подключает существующий курс.
 * - createStub → создаёт stub-курс (1 секция + 1 урок) и линкует.
 */
  ...
  for (const n of nodes) {
    ...
    if (n.existingCourseSlug) {
      ...
      if (existing.docs.length > 0) {
        ...
        // Fallback: если реального курса ещё нет в БД (seed.ts не запускался),
        // создаём stub, чтобы узел всё равно был кликабельным. Когда seed.ts
        // потом создаст настоящий курс, админ может перелинковать узел через
        // Payload admin — или можно повторно запустить seed-roadmaps.ts.
        console.warn(
      ...
      const stub = await createStubCourse(payload, {
        ...
        slug: `rm-${n.nodeId}`,
        roadmapId,
        order: 1000 + n.order, // stub-курсы идут после реальных в списке
      })
      courseId = stub.id
    }
